...
- Additives: three flag states only — "No known concern" (neutral, NOT green) / "Worth knowing" (amber tint) / "Documented concern" (avoid tint), each non-neutral flag citing its paper. **No hazard spectrum, no invented gradients** — colour states what's documented, nothing more. **Sweeteners are NOT flagged** (owner, 20 August 2026, CHK-E5): the earlier "always at least Worth knowing" policy was dropped when its WHO-2023 basis was verified and found to be about weight control and disease risk, conditional, and silent on sweeteners in a nightly supplement. Fear-based "no nasty sweeteners" marketing is penalised as marketing dishonesty instead. This is the rule working: a flag whose source does not carry it does not ship.
...
## Interface economy
- **Chrome scales with catalogue size.** Under ~20 items: no filter row, no sort — the list is the interface; order declared in prose. Above: Form as one derived dropdown (options computed from the page's data, with counts; zero-result options never render), plus genuine-preference filters (Independently tested, No artificial sweeteners) that only appear when both states exist. No sort controls; the fixed order is the VERDICT (delivers → matches-but-unverified → not-what-was-studied), then checks-passed WITHIN a verdict, then brand A–Z as the documented tiebreak — owner-ratified 18 August 2026, superseding the earlier "checks-passed is the fixed order", which was written when the verdict was still a count. Ordering on the raw count inverts the ruled verdict: a bottle failing dose-match but passing the other three (3 checks, "not what was studied") would sort above one whose core pair holds but whose trust checks fail (2 checks, "matches, but unverified"). See CHK-Rprod.4. Remedies page keeps its outcome filters (31 items, routing) with Grouped/A–Z as a view toggle.
...
- Real audit data only. *(**RE-DERIVED from `sources[]` at CHK-E2, 20 August 2026**, replacing the design-phase figures that were counted on a wider basis (kava read 5 there, 3 in-repo; melatonin 12 vs 8). All 133 sources are now adjudicated for `measuresSleepOutcome`, so these are the counts the bars actually render. Format: cited / measured a sleep outcome / reported enough to verify.)* melatonin 8/5/3 · saffron 7/5/0 · valerian 5/4/0 · magnesium 4/2/0 · L-theanine 2/2/0 · chamomile 2/2/0 · ashwagandha 4/2/0 · kava 3/0/0 · bacopa 7/1/1 · taurine 5/0/0. **The third figure is the open gap: 6 of 65 sleep-outcome sources carry the sample size and direction that "verifiable" requires** — that fill is the rest of CHK-E2.
