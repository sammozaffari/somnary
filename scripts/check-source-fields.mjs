 ...
 *
 * The study field is a NESTED BAR (RULES.md "Evidence display"): three counts — cited ⊇
 * measured-a-sleep-outcome ⊇ reported-enough-to-verify — plus one plain direction sentence
 * from `effectDirection`. No scatter, no dots, no per-study positioning.
 ...
 *   - complete + measuresSleepOutcome=true but missing sampleSize / effectDirection
 *
 * A `pending` source (any fields null) is VALID — honestly flagged incomplete; the nested bar
 * counts it outside "reported enough to verify". `effectSize` is always optional (stat line
 * only, never positioning). `studyQuality` is always optional too — populated only for
 * bucket-determining papers per the re-scoped docs/SOURCE_QUALITY_RUBRIC.md.
 */
...
async function main() {
    ...
    for (const s of data.sources || []) {
      ...
      if (s.effectDataStatus === 'complete') {
        if (s.measuresSleepOutcome === null || s.measuresSleepOutcome === undefined) {
          ...
          // studyQuality intentionally absent (RULES.md Process: rubric scoped to bucket-determining papers)
          const miss = ['sampleSize', 'effectDirection'].filter((k) => !set(s[k]));
  ...
  for (const k of ['sampleSize', 'effectDirection'])
    console.log(`    ${k.padEnd(14)} ${String(eff[k]).padStart(4)} / ${efficacy}  ${pctOf(eff[k], efficacy)}   (required on complete)`);
  console.log(`    ${'studyQuality'.padEnd(14)} ${String(eff.studyQuality).padStart(4)} / ${efficacy}  ${pctOf(eff.studyQuality, efficacy)}   (optional — bucket-determining papers only)`);
