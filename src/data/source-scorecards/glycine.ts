...
export const GLYCINE_SOURCES: GlyProduct[] = [
  ...
  // ── Phase 2: Amazon AU / iHerb bestsellers + an AU-domestic powder (deduped). Ratified 2026-07-22.
  // Glycine is a thin AU category: of 12 scouted bestsellers, 6 were excluded (magnesium-glycinate or
  // no-glycine combos, capsule-only when a powder was sought, or not reachable to an AU buyer). These 6 are
  // the genuine, AU-reachable, standalone-glycine additions.
  {
    ...
    productName: 'Glycine Powder',
    bottomLine:
      'The only glycine here with a public per-lot lab-report lookup — but it takes three scoops to reach the studied dose, and there’s no product certification.',
    form: 'single-ingredient powder',
    dose: '1 g per scoop',
    doseReach: { note: '~3 scoops to reach the studied ~3 g bedtime dose', reaches: false },
    regulated: 'food',
    ausL: null,
    channel: 'Amazon AU (import)',
    certSummary: 'Food — not TGA-listed (import)',
    ...
    scores: {
      testing_purity: { score: 0, note: 'No finished-product third-party certification (USP/NSF/Informed); the testing program is the maker’s own, not an independent product cert.' },
      label_accuracy: { score: 1, note: 'Runs a public per-lot lab-report lookup (enter the lot number) — the only glycine here with an openable-report mechanism — but coverage of the glycine SKU’s lots is unconfirmed, so credited as at-least-on-request (owner-ratified).' },
      additives: { score: 5, note: 'Pure single-ingredient glycine; no additive of concern.' },
      regulatory: { score: 5, note: 'No FDA/FTC action names this product or line.' },
      transparency: { score: 4, note: 'Named maker + glycine dose disclosed + a real per-lot lab-report program (a genuine verification signal beyond a bare self-claim); still a food import with no AUST L.' },
      marketing_honesty: { score: 5, note: 'Testing-led copy ("third-party tested / transparent labels"); no grade overclaim and no implied certification.' },
    },
    verdict:
      'The strongest verification posture of any glycine here — a public per-lot lab-report lookup — held back only because the glycine SKU’s coverage is unconfirmed, it takes three scoops to reach the studied dose, and there is no independent product certification.',
    heldItems: ['Whether the glycine SKU’s lots resolve to an openable lab report — confirm from a physical bottle’s lot number'],
  ...
  },
  {
    ...
    productName: 'Glycine Powder',
    bottomLine:
      'A pure powder that reaches the studied ~3 g in a single serving — but its "third-party tested" line has no openable lab report, and no product certification.',
    form: 'single-ingredient powder',
    dose: '3 g per serving',
    doseReach: { note: 'A single serving reaches the studied ~3 g bedtime dose', reaches: true },
    regulated: 'food',
    ausL: null,
    ...
    scores: {
      testing_purity: { score: 0, note: 'No finished-product third-party certification and no published lab test; a GMP facility is not a product cert.' },
      label_accuracy: { score: 0, note: 'No independent lab test of the exact product; no public or openable per-lot lab report (site report path did not resolve).' },
      additives: { score: 5, note: 'Pure single-ingredient glycine; no additive of concern.' },
      regulatory: { score: 5, note: 'No FDA/FTC action names this product or line.' },
      transparency: { score: 3, note: 'Named maker + glycine dose disclosed (3 g/serving, reaches the studied dose) — but its "third-party tested" line is a bare self-claim with no openable report, and it’s a food import with no AUST L.' },
      marketing_honesty: { score: 5, note: 'The soft "third-party tested" wording is not an explicit certification claim, so it is not docked (consistent with the Nutricost ashwagandha ruling).' },
    },
    verdict:
      'One of the few here to reach the studied ~3 g in a single serving from a clean pure powder — but "third-party tested" is a self-claim with no openable lab report, and there is no product certification and no TGA listing.',
    heldItems: [],
    communityStatus: 'gathering',
    ratified: true,
  },
  {
    ...
    productName: 'Glycine',
    bottomLine:
      'The cleanest capsule panel here — veg cap, almost nothing else — but it makes no testing claim, and takes three capsules to reach the studied dose.',
    form: 'single-ingredient capsules',
    dose: '1 g per capsule',
    doseReach: { note: '3 capsules to reach the studied ~3 g bedtime dose', reaches: false },
    regulated: 'food',
    ausL: null,
    ...
    scores: {
      testing_purity: { score: 0, note: 'No third-party product cert, no published lab test, no public batch report.' },
      label_accuracy: { score: 0, note: 'No independent lab test of the exact product; no public or openable per-lot report located.' },
      additives: { score: 5, note: 'The cleanest panel in the set — vegetarian capsule + ascorbyl palmitate only (no magnesium stearate or silica); no additive of concern.' },
      regulatory: { score: 5, note: 'No regulator action names this glycine product; a 2017 FDA warning letter named the entity for website disease claims only, not this product or its quality — held.' },
      transparency: { score: 3, note: 'Named maker + glycine dose disclosed (1 g/cap) + a fully disclosed clean panel — but it makes no grade or testing claim, and it’s a food import with no TGA listing.' },
      marketing_honesty: { score: 5, note: 'Restrained structure/function copy; no grade/cert claim to overstate.' },
    },
    verdict:
      'An identifiable maker with the cleanest disclosed capsule panel of the set (veg cap + ascorbyl palmitate only) — but it makes no testing claim, reaching the studied ~3 g takes three capsules, and there’s no independent testing.',
    heldItems: ['A 2017 FDA warning letter named the entity for website disease claims only — not this product or its quality; held'],
    communityStatus: 'gathering',
    ratified: true,
  },
  {
    ...
    productName: 'Glycine',
    bottomLine:
      'A quiet, no-claims capsule — but it’s a gelatin (non-veg) cap, takes six capsules to reach the studied dose, and has no independent testing.',
    form: 'single-ingredient capsules',
    dose: '500 mg per capsule',
    doseReach: { note: '6 capsules to reach the studied ~3 g (tied heaviest load)', reaches: false },
    regulated: 'food',
    ausL: null,
    channel: 'iHerb AU · Amazon AU (import)',
    certSummary: 'Food — not TGA-listed (import)',
    ...
    scores: {
      testing_purity: { score: 0, note: 'No third-party product cert, no published lab test, no public batch report.' },
      label_accuracy: { score: 0, note: 'No independent lab test of the exact product; no public or on-request report located.' },
      additives: { score: 5, note: 'Disclosed benign panel (gelatin capsule, magnesium stearate, microcrystalline cellulose, silica); no additive of concern, though the gelatin capsule is non-vegetarian.' },
      regulatory: { score: 5, note: 'No regulator action names this glycine product; an unrelated "One Source Nutrition" recall is a different company — held.' },
      transparency: { score: 3, note: 'Named maker + glycine dose disclosed (500 mg/cap); makes no grade claim, and it’s a food import with no TGA listing.' },
      marketing_honesty: { score: 5, note: 'Restrained copy; no grade/cert claim to overstate.' },
    },
    verdict:
      'A named maker with a disclosed, benign panel that makes no grade claim — but it uses a non-vegetarian gelatin capsule, reaching the studied ~3 g takes six capsules, and there’s no independent testing.',
    heldItems: ['An unrelated "One Source Nutrition" recall is a different company — held'],
    communityStatus: 'gathering',
    ratified: true,
  },
  {
    ...
    productName: 'Glycine',
    bottomLine:
      'The most genuinely Australian option here — but sold as a sports food with no TGA listing, an "independently tested" badge with no linked report, and three servings to reach the studied dose.',
    form: 'single-ingredient powder',
    dose: '1 g per serving',
    doseReach: { note: '~3 servings to reach the studied ~3 g bedtime dose', reaches: false },
    regulated: 'food',
    ausL: null,
    ...
    scores: {
      testing_purity: { score: 0, note: 'No finished-product third-party certification and no published lab test; the "Independently Tested" badge is a claim, not a document.' },
      label_accuracy: { score: 0, note: 'No independent lab test of the exact product is public; results are on-request only ("send us a message").' },
      additives: { score: 5, note: 'Pure single-ingredient glycine; no additive of concern.' },
      regulatory: { score: 5, note: 'No TGA/ACCC action names this product or maker.' },
      transparency: { score: 3, note: 'Named AU maker + glycine dose disclosed — but the "Independently Tested" badge links to no report, and it is a "Formulated Supplementary Sports Food" with no AUST L (outside TGA medicine oversight).' },
      marketing_honesty: { score: 5, note: 'The soft "Independently Tested" badge is not an explicit certification claim, so it is not docked (consistent with the other testing-claim rulings).' },
    },
    verdict:
      'The most genuinely Australian option of the set (AU-made, AU shipping), but sold as a sports food outside TGA medicine oversight, with an "Independently Tested" badge that links to no report and a serving that reaches the studied ~3 g only in triplicate.',
    heldItems: [],
    communityStatus: 'gathering',
    ratified: true,
  },
  {
    ...
    productName: 'Glycine Powder',
    bottomLine:
      'Headlines itself the "best for sleep," but a label serving is a third of the studied dose, there’s no openable lab report, and no product certification.',
    form: 'single-ingredient powder',
    dose: '1 g per serving',
    doseReach: { note: 'A label serving is 1 g — about a third of the studied ~3 g; it takes ~3 servings to reach it', reaches: false },
    regulated: 'food',
    ausL: null,
    channel: 'Amazon AU · iHerb AU (import)',
    certSummary: 'Food — not TGA-listed (import)',
    ...
    scores: {
      testing_purity: { score: 0, note: 'No finished-product third-party certification and no published lab test; a testing claim with no openable document.' },
      label_accuracy: { score: 0, note: 'No independent lab test of the exact product; no public or openable per-lot report located.' },
      additives: { score: 5, note: 'Pure single-ingredient glycine; no additive of concern.' },
      regulatory: { score: 5, note: 'No FDA/FTC action names this product or line.' },
      transparency: { score: 3, note: 'Named maker + glycine dose disclosed (1 g/serving) — but no openable report, and a food import with no AUST L.' },
      marketing_honesty: { score: 4, note: 'Headlines "Best Glycine Supplement for Sleep" — an unsubstantiated superlative sleep claim, paired with a label serving (1 g) that is a third of the studied dose (owner-ratified −1).' },
    },
    verdict:
      'A pure glycine powder undercut by its own marketing: it headlines itself the "best for sleep" while a label serving delivers a third of the studied dose, with no openable lab report and no product certification.',
    heldItems: [],
    communityStatus: 'gathering',
    ratified: true,
  },
